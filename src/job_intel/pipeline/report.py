    ...
    for item in results:
        lines.extend(
            [
                ...
                f"- LLM fit: {item.llm_score:.1f}" if item.llm_score is not None else "- LLM fit: -",
                f"- Location: {item.location or '-'}",
                f"- URL: {item.url or '-'}",
                f"- LLM recommendation: {item.llm_recommendation or '-'}",
                f"- LLM concerns: {', '.join(item.llm_concerns or []) or '-'}",
