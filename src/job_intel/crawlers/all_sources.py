...
class AllSourcesCrawler(JobCrawler):
    source = "all"
    ...
    def crawl(self) -> list[JobPosting]:
        crawlers: list[JobCrawler] = [
            ...
            RemoteOkCrawler(limit=self.limit_per_source),
            RemotiveCrawler(limit=self.limit_per_source),
            HimalayasCrawler(limit=self.limit_per_source),
            ArbeitnowCrawler(limit=self.limit_per_source),
        ]
